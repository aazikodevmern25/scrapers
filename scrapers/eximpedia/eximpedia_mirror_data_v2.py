    ...
    try:
        ...
        for country in countries:
            try:
                ...
                # Mirror Data searches all trade types - try import, export, and exim
                country_records = []
                modes_to_try = ['import', 'export', 'exim']
                
                for mode in modes_to_try:
                    logger.info(f"  Fetching {mode.upper()} data for {country}")
                    
                    # Get taxonomy data for this country and mode
                    taxonomy_info = GetTaxonomyData(sess, country, mode)
                    if not taxonomy_info:
                        logger.info(f"  No {mode} data available for {country}, skipping this mode")
                        continue
                    
                    # Fetch ALL records with pagination for this mode (no limit!)
                    mode_records = []
                    page_start = 0
                    page_size = 1000
                    total_records = None
                    ...
                    while True:
                        ...
                        # Add country and mode info to each record
                        ...
                        # Check if we've retrieved all records for this mode
                        page_start += page_size
                        if len(mode_records) >= total_records:
                            break
                        
                        # NO MAX LIMIT - fetch ALL records like regular Eximpedia download
